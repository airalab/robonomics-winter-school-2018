              ...
              <v-container grid-list-md class="px-3">
                <v-layout row wrap>
                  <v-flex md12 class="text-xs-center">
                    ...
                    <div v-if="agents.list.length > 0" class="my-3">
                      Agents:
                      <a :href="agents.url" target="_blank">{{agents.list.length}}</a>
                    </div>
                    <v-alert v-if="modelError!==''" :value="true" type="error">{{modelError}}</v-alert>
                    <v-btn
                      v-if="modelError===''"
          ...
          robonomics.onResult(msg => {
            ...
            if (
              ...
              if (
                ...
                _indexOf(this.agents.list, msg.sender) < 0
              ) {
                console.log("Skip result of", msg.sender);
                return;
              }
