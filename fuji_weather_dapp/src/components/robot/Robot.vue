      ...
      <v-layout justify-center row wrap>
        <v-flex sm12 md10 lg6>
          <v-card>
            ...
            <v-card-text>
              <v-select v-model="objective" :items="objectives" label="Objective"></v-select>
        ...
        <v-flex sm12 md10 lg6>
          <v-card>
            ...
            <v-card-text>
              ...
              <v-btn color="primary" @click.native="show">Show</v-btn>
            </v-card-text>
          </v-card>
        </v-flex>
      </v-layout>
    ...
    return (
      getIpfs()
        ...
        .then(() => {
          ...
          robonomics.onDemand(config.RUN.model, msg => {
            console.log("demand", msg);
            // emulator kfc
            // return this.emulatorKfc(msg)
          });
          robonomics.onOffer(config.RUN.model, msg => {
    ...
    show() {
      if (this.$refs.liabilityForm.$refs.form.validate()) {
        const liability = new Liability(
          robonomics.web3,
          this.$refs.liabilityForm.address,
          ""
        );
        this.newLiability(liability);
