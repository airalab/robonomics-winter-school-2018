          ...
          <div style="border: 1px solid #6b6060; height: 200px; overflow-y: scroll;">
            <ol>
              <li
                ...
                :key="i"
                style="padding: 5px;"
                :style="{ backgroundColor: (log.type === 1) ? '#f8f8f8' : '#eeeeee' }"
              >{{log.msg}}</li>
            </ol>
          </div>
        </div>
          ...
          return rosBag(
            ...
            bag => {
              if (bag.topic === "/objective") {
                actionId = _findIndex(this.actions, {
                  objective: bag.message.data
                });
              }
            },
            { topics: ["/objective"] }
